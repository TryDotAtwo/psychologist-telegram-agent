...
export async function fetchSiteAsset(request: Request, env: Env): Promise<Response> {
  ...
  if (url.pathname === "/" || url.pathname === "") url.pathname = "/site/index.html";
  else if (url.pathname === "/articles" || url.pathname.startsWith("/articles/")) url.pathname = "/site/index.html";
  else if (!url.pathname.startsWith("/site/")) url.pathname = `/site${url.pathname}`;

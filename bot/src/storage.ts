...
export const DEFAULT_SITE_CONFIG: SiteConfig = {
  ...
  consentText:
    "Я даю согласие на обработку данных, которые самостоятельно передаю через сайт: имя или псевдоним, контакт, сообщение в чате, запрос на запись и выбранное время. Данные нужны, чтобы ответить на обращение, записать на консультацию и сохранить понятный контекст диалога. Я понимаю, что сайт и бот не являются экстренной помощью, не ставят диагноз и не заменяют врача.",
  privacyText:
    "Сайт использует техническую cookie на 24 часа. Она содержит только случайный идентификатор и не хранит имя, диагнозы или переписку. Читать сайт и блог можно без согласия. Чат, запись, связь с Telegram и хранение профиля включаются только после явного согласия. Сайт и Telegram-бот обрабатывают данные только для ответа на запрос, записи на консультацию, ведения диалога, напоминаний и непрерывности сопровождения. Данные не публикуются, не попадают в статьи и не используются в публичном блоге. Исходный код доступен на GitHub.",
  articleAgentInstructions:
    "Пиши статьи для взрослых нейроотличных людей спокойным русским языком. Опирайся только на публичные материалы сайта и тему, заданную администратором. Не читай клиентские переписки, профили, заметки, напоминания или приватные данные. Не ставь диагнозы, не обещай лечение, не используй эзотерику и маркетинговые клише. Объясняй, где заканчивается самонаблюдение и начинается необходимость очной помощи.",
